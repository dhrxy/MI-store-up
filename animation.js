...
/**
 * 
 * @param {object} ele 标签对象
 * @param {object} json json对象-----必须有data属性（需要操作的对应的属性）
 *  [option]  默认匀速运动 
 *    [easing]   运动函数（默认匀速运动）
 ...
 *      1   代表减速运动
 *      2   先加速后减速
 * @param {number} time 运动的时间间隔
 * @param {function} callback 运动结束的回调函数
 */
...
function animation(ele,json,time,callback){
	...
	function fn(){
		...
		for (var key in changeValue) {
			...
			if (T_ <= 0) { 
				val = Math.min(parseFloat(data[key]),val);
				val = Math.max(parseFloat(data[key]),val);
			}
